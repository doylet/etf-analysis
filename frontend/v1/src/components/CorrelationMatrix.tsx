...
export default function CorrelationMatrixComponent({ 
  portfolioId, 
  defaultTimeWindow = 252 
}: CorrelationMatrixProps) {
  const [timeWindowDays, setTimeWindowDays] = useState(defaultTimeWindow);
  
  const { data: matrix, loading, error, metadata, cacheHit } = useCorrelationMatrix({
    portfolioId,
    timeWindowDays,
  });
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <MetricCard
              title="Time Period"
              value={`${matrix?.time_period?.days || 0} days`}
              subtitle={`${formatDate(matrix?.time_period?.start_date || '')} - ${formatDate(matrix?.time_period?.end_date || '')}`}
              icon={Calendar}
              variant="subtle"
              trend="neutral"
            />

...
from src.widgets.layered_base_widget import LayeredBaseWidget
...
class CorrelationMatrixWidget(LayeredBaseWidget):
    ...
    def _render_holdings_selection(self, holdings: List[Dict]) -> List[str]:
        ...
        with st.expander(f"Portfolio Holdings ({selected_count}/{total_count} selected)", expanded=True):
            ...
            with col1:
                if st.button("Select All", key=self._get_session_key("select_all_holdings")):
                    st.session_state[key] = [h['symbol'] for h in holdings]
                    st.rerun()
            with col2:
                if st.button("Deselect All", key=self._get_session_key("deselect_all_holdings")):
            ...
            for idx, holding in enumerate(holdings):
                ...
                with cols[col_idx]:
                    is_selected = st.checkbox(
                        holding['symbol'],
                        value=holding['symbol'] in st.session_state[key],
                        key=self._get_session_key(f"holding_{holding['symbol']}")
                    )
                    if is_selected:
                        selected_holdings.append(holding['symbol'])
            
            st.session_state[key] = selected_holdings
    ...
    def _render_benchmark_selection(self) -> List[str]:
        ...
        with st.expander(f"Benchmark Instruments ({selected_count}/{total_count} selected)", expanded=False):
            ...
            col1, col2, col3 = st.columns([1, 1, 4])
            with col1:
                if st.button("Select All", key=self._get_session_key("select_all_benchmarks")):
                    ...
                    st.rerun()
            with col2:
                if st.button("Deselect All", key=self._get_session_key("deselect_all_benchmarks")):
                    custom_symbols = [s for s in current_additional if s not in self.AVAILABLE_INSTRUMENTS]
    ...
    def _render_custom_symbols(self):
        ...
        with st.expander(f"Custom Symbols ({len(custom_symbols)})", expanded=True):
            # Display existing custom symbols
            if custom_symbols:
                ...
                for symbol in custom_symbols:
                    ...
                    with col1:
                        st.text(symbol)
                    with col2:
                        if st.button("×", key=self._get_session_key(f"remove_{symbol}"), help=f"Remove {symbol}"):
            ...
            with col1:
                st.text_input(
                    "Enter symbol:",
                    key=self._get_session_key("custom_symbol"),
                    placeholder="e.g., AAPL, MSFT, VEU.AX"
                )
            with col2:
                st.markdown("&nbsp;")
                if st.button("Add", key=self._get_session_key("add_custom")):
                    self._handle_add_custom_symbol()
    
    def _handle_add_custom_symbol(self):
        """Handle custom symbol addition with validation."""
        symbol_input = st.session_state.get(self._get_session_key("custom_symbol"), "").upper().strip()
        
        # Validate
        validation_result = self._validate_custom_symbol(symbol_input, st.session_state.get(self._get_session_key("selected_additional"), []))
    ...
    def _render_correlation_statistics(self, analysis: CorrelationAnalysis):
        ...
        with col1:
            ...
            if analysis.avg_correlation > 0.7:
                ...
                st.info("ℹ️ Moderate correlation - reasonable diversification")
            else:
    ...
    def _render_key_pairs(self, pairs_df: pd.DataFrame):
        ...
        with col1:
            ...
            st.dataframe(pairs_df.head(5), hide_index=True, use_container_width=True)
        
        with col2:
            st.markdown("**Least Correlated Pairs:**")
            st.dataframe(pairs_df.tail(5), hide_index=True, use_container_width=True)
    ...
    @staticmethod
    def _validate_custom_symbol(symbol: str, existing_symbols: List[str]) -> Dict:
        """
        ...
        Returns:
            Dict with 'valid', 'message', and 'error_type' keys
        """
        # Check non-empty
        if not symbol:
            return {'valid': False, 'message': 'Please enter a symbol', 'error_type': 'warning'}
        
        # Check length
        if len(symbol) < 1 or len(symbol) > 10:
            return {'valid': False, 'message': 'Symbol must be between 1 and 10 characters', 'error_type': 'error'}
        
        # Check format
        if not re.match(r'^[A-Z0-9.\-]+$', symbol):
            return {'valid': False, 'message': 'Symbol must contain only uppercase letters, numbers, dots, and dashes', 'error_type': 'error'}
        
        # Check duplicates
        if symbol in existing_symbols:
            return {'valid': False, 'message': f'{symbol} is already selected', 'error_type': 'warning'}
